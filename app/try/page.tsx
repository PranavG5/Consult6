  ...
  if (alreadyUsed && state !== "done") {
    return (
      <div style={{ minHeight: "100vh", background: "#1a1a1a", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ background: "#242424", border: "1px solid #333", borderRadius: 14, padding: 40, maxWidth: 420, textAlign: "center" }}>
          ...
          <Link href="/auth/login" style={{ display: "block", background: "#1e1e1e", border: "1px solid #333", color: "#ccc", fontSize: 14, fontWeight: 600, textDecoration: "none", padding: "11px 0", borderRadius: 9 }}>Sign In</Link>
        </div>
      </div>
    );
  }
        ...
        <div style={{ background: "#242424", border: "1px solid #333", borderRadius: 12, padding: 28 }}>
          {state === "done" && (
            <div style={{ marginBottom: 20 }}>
              <button onClick={() => { setState("idle"); setAnalysis(null); setFiles([]); setOrgName(""); setProgress(0); }}
                style={{ width: "100%", background: "#1e1e1e", border: "1px solid #444", color: "#ccc", borderRadius: 9, padding: "11px 0", fontSize: 14, fontWeight: 600 }}>
                ← New Analysis
              </button>
            </div>
          )}
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {files.length === 0 && !isRunning && state !== "done" && (
              <div onDrop={handleDrop} onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)}
                ...
                style={{ border: `2px dashed ${dragging ? "#CC5500" : "#333"}`, borderRadius: 10, padding: "28px 20px", textAlign: "center", cursor: "pointer", background: dragging ? "#2a1800" : "transparent" }}>
          ...
          {state === "done" && analysis && (
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ background: "#1e1e1e", border: "1px solid #333", borderRadius: 8, padding: "12px 14px" }}>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: "#242424", border: "1px solid #333", borderRadius: 16, padding: 36, maxWidth: 400, width: "100%", textAlign: "center" }}>
            ...
            <Link href="/auth/login" style={{ display: "block", background: "#1e1e1e", border: "1px solid #333", color: "#ccc", fontSize: 14, fontWeight: 600, textDecoration: "none", padding: "11px 0", borderRadius: 9, marginBottom: 16 }}>
              Sign In Instead
            </Link>
